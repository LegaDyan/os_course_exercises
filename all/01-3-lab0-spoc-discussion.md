...
- 你理解的对于类似ucore这样需要进程/虚存/文件系统的操作系统，在硬件设计上至少需要有哪些直接的支持？至少应该提供哪些功能的特权指令？
  ...
  硬件上需要有时钟中断支持、有存储介质等；需要有对文件读写、特殊寄存器的读写等特殊指令。

- 你理解的x86的实模式和保护模式有什么区别？物理地址、线性地址、逻辑地址的含义分别是什么？

  保护模式和实模式主要是指内存访问时使用虚拟地址还是真实地址。在保护模式下，程序只能访问操作系统提供的虚拟地址，而无法对物理地址进行直接操作；反之，实模式可以直接操作物理地址。

  - 物理地址是指处理器实际放在地址总线上的地址；
  - 线性地址是逻辑地址到物理地址转换的中间层，是处理器通过段机制控制下形成的地址空间；
  - 逻辑地址是指程序指令在访问内存时给出的地址。
